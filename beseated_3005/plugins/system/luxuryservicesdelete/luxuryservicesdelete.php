...
class PlgSystemLuxuryservicesdelete extends JPlugin
{
	public function onAfterInitialise()
	{
		...
		$usergroups = $this->getUserGroup($user->groups);

		$session             = JFactory::getSession();
	    $yacht_service_saved =  $session->get( 'yacht_service_saved',0);
	    $chauffeur_service_saved =  $session->get( 'chauffeur_service_saved',0);

		if(in_array('Yacht', $usergroups))
		{
		    if($detail_page == 0)
		    {
		...
		if(in_array('Chauffeur', $usergroups))
		{
			if($detail_page == 0)
		    {
			    if($chauffeur_service_saved)
			    {
			    	$session->set( 'chauffeur_service_saved', 0);

					setcookie('chauffeur_ser_default_img_id', null, -1);
					setcookie('chauffeur_new_ser_img_id', null, -1);
					setcookie('page_load', null, -1);
					setcookie('chauffeur_service_saved', null, -1);
			    }
			    else
			    {
			    	if(isset($_COOKIE['chauffeur_new_ser_img_id']) && !empty($_COOKIE['chauffeur_new_ser_img_id']))
			    	{
			    		$new_image_ids = implode(',',json_decode($_COOKIE['chauffeur_new_ser_img_id']));
					...
					if(isset($_COOKIE['chauffeur_ser_default_img_id']) && !empty($_COOKIE['chauffeur_ser_default_img_id']))
	    			{
	  					$def_image_id = $_COOKIE['chauffeur_ser_default_img_id'];
					...
					if(!isset($_COOKIE['chauffeur_ser_default_img_id']) && $service_id)
					{
						$tblService = JTable::getInstance('ChauffeurService', 'BeseatedTable');
						$tblService->load($service_id);
						$tblService->thumb_image = '';
				...
				setcookie('page_load', null, -1);
				setcookie('service_id', null, -1);
				unset($_COOKIE['chauffeur_new_ser_img_id']);
				setcookie('chauffeur_new_ser_img_id', null, -1);
				unset($_COOKIE['chauffeur_ser_default_img_id']);
				setcookie('chauffeur_ser_default_img_id', null, -1);
			}
		}
	}
	...
	{
		$usergroupIDS = implode(',', $usergroups);
		...
		$query->select('title')
			...
			->where($db->quoteName('id') . ' IN ('.$usergroupIDS.')');
