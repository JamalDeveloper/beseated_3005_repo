...
class plgSystemSourcererHelper
{
	...
	{
		NNProtect::protectForm($str, $this->src_params->protected_tags);
	}

	function protectTags(&$str)
	{
		NNProtect::protectTags($str, $this->src_params->protected_tags);
	}

	function unprotectTags(&$str)
	{
		NNProtect::unprotectTags($str, $this->src_params->protected_tags);
	}
	...
	function cleanLeftoverJunk(&$str)
	{
		...
		if (strpos($str, '{/' . $this->src_params->syntax_word) !== false)
		{
			...
			$str = preg_replace($regex, '<!-- ' . JText::_('SRC_COMMENT') . ': ' . JText::_('SRC_CODE_REMOVED_NOT_ENABLED') . ' -->', $str);
		}
	}
}
