...
class plgBeseatedCcavenue extends JPlugin
{
	...
	function onCCAvenuePayment()
	{
		...
		else if (strtolower($bookingType == 'venue.confirm'))
		{
			$tblVenueBooking = JTable::getInstance('VenueBooking', 'BeseatedTable');
			$tblVenueBooking->load($bookingID);
			$totalPrice           = $tblVenueBooking->pay_deposite;
			$currencyCode         = $tblVenueBooking->booking_currency_code;
		}
		...
		}else if(strtolower($bookingType) == 'venue' || strtolower($bookingType) == 'venue.confirm'){
		...
		$redirectUrl = JURI::base() . 'index.php?option=com_beseated&payment_id='.$tblPayment->payment_id.'&task=payment.ipn&source='.$option;
