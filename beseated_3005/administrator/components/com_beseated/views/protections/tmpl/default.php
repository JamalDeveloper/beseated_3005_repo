...
?>
<form action="index.php?option=com_beseated&view=protections" method="post" id="adminForm" name="adminForm">
...
?>
	<table class="table table-striped table-hover">
		...
		<tbody>
			<?php if (!empty($this->protection)) : ?>
				<?php foreach ($this->protection as $i => $row) :
					$link = JRoute::_('index.php?option=com_beseated&view=protection&layout=edit&protection_id=' . $row->protection_id);
				?>
					<tr>
						<td><?php echo $this->pagination->getRowOffset($i); ?></td>
						<td>
							<?php echo JHtml::_('grid.id', $i, $row->protection_id); ?>
						</td>
						<td width="2%" style="min-width:55px" class="">
							<?php echo JHtml::_('jgrid.published', $row->published, $i, 'protections.', true, 'cb'); ?>
						</td>
						<td>
							<a href="<?php echo $link ?>">
								<?php echo $row->protection_name; ?>
							</a>
						</td>
						...
						<td class="">
							<?php if(isset($this->revenues[$row->protection_id])): ?>
								<?php echo number_format($this->revenues[$row->protection_id],2); ?>
							<?php else: ?>
								0.00
							<?php endif; ?>
						</td>
