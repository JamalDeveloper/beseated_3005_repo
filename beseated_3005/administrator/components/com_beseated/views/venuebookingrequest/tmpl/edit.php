...
<script type="text/javascript">
    function initialize()
    {
        ...
        jQuery(document).ready(function()
        {
          getTableCapacity();
            ...
            jQuery.ajax({
                        url:'index.php?option=com_beseated&task=venuebookingrequest.getVenueServcies',
                        type: "POST",
                        data: {
                            'venue_id': venue_id
                        }
            ...
            jQuery.ajax({
                        url:'index.php?option=com_beseated&task=venuebookingrequest.isDayClub',
                        type: "POST",
                        data: {
                            'venue_id': venue_id
                        }
     ...
     Joomla.submitbutton = function(task)
    {
        ...
        if(task !== 'venuebookingrequest.cancel')
        {
            if (isValid)
            {
                if (useDate < fullDate)
...
<form action="<?php echo JRoute::_('index.php?option=com_beseated&view=venuebookingrequest&layout=edit&venue_table_booking_id=' . (int) $this->item->venue_table_booking_id); ?>"
    method="post" name="adminForm" id="adminForm" >
    <div class="form-horizontal">
        <fieldset>
            <div class="span12">
                <legend><?php echo JText::_('COM_BESEATED_MANAGE_VENUE_TABLE_BOOKING_REQUEST_DETAIL'); ?></legend>
