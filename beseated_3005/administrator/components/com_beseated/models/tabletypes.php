 ...
 * @since  1.6
 */
class BeseatedModelTableTypes extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				'table_type_id', 'a.`table_type_id`',
				'table_type_name', 'a.`table_type_name`',
				'table_type_desc', 'a.`table_type_desc`',
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.table_type_id', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__beseated_venue_table_type` AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.`table_type_id` LIKE ' . $search . '  OR  a.`table_type_name` LIKE ' . $search . ' )');
			}
		}
