...
class BeseatedModelEvent extends JModelAdmin
{
	...
	protected function loadFormData()
	{
        ...
        if($elementID)
        {
        	...
        	// Create the base select statement.
        	$query->select('SUM(available_tickets) as available_ticket,SUM(total_tickets) as total_ticket')
        		->from($db->quoteName('#__beseated_event_ticket_type_zone'))
        		->where($db->quoteName('is_deleted') . ' = ' . $db->quote('0'))
        		->where($db->quoteName('event_id') . ' = ' . $db->quote($elementID));
	...
	public function save($data)
	{
		...
		$deleteCookie = $this->unset_cookie('event_ticket_type_id');
		...
		$this->updateTicketsTypeZoneAfterSave($input,$table->event_id);
	...
	public function updateTicketsTypeZoneAfterSave($input,$event_id)
	{
		...
		foreach ($_FILES['filedata'] as $fieldName => $fieldIndexs) 
		{
			$i = 0;
			foreach ($fieldIndexs as $fieldValue) 
			{
				$imgData[$i][$fieldName] = $fieldValue;
				$i++;
			}
		}
		...
		{ 
			...
			if(!empty($imgData[$i]['name']))
			{
				$imagePath = $this->uploadTicketTypeImages($event_id,$imgData[$i]);
