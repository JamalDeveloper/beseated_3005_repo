 ...
 * @since  0.0.1
 */
class BeseatedModelPremiumTables extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'premium_table_name','a.premium_table_name'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')
			->from($db->quoteName('#__beseated_venue_premium_table','a'));


		if (!empty($search))
		{
			$like = $db->quote('%' . $search . '%');
			$query->where('a.premium_table_name LIKE ' . $like);
		}
		...
		{
			$fullordering = "a.premium_table_name ASC";
		}
	...
	protected function populateState($ordering = 'id', $direction = 'ASC')
	{
		...
		$orderCol = $app->input->get('filter_order', 'a.premium_id');

		if (!in_array($orderCol, $this->filter_fields))
		{
			$orderCol = 'a.premium_id';
		}
