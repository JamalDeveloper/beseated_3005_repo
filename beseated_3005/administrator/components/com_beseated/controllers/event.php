...
class BeseatedControllerEvent extends JControllerForm
{
	...
	public function upload_images()
	{
		...
		$ticket_type_id = $input->get('uploadTickets',0,'int');
		...
		if(isset($_FILES["Filedata"]))
		{
			...
			for($i=0; $i < $fileCount; $i++)
			{
				...
				//$totalTicket = $this->getTotalTickets($elementID); // jamal 28-09-16
			...
			$tableTicketTypeZone->load($ticket_type_id);
			$tableTicketTypeZone->total_tickets = $tableTicketTypeZone->total_tickets + count($imageIDs);
			$tableTicketTypeZone->available_tickets = $tableTicketTypeZone->available_tickets + count($imageIDs);
	...
	public function cancel()
	{
		...
		//echo "<pre/>";print_r($ticket_types);exit;
	    ...
	    if(isset($event_id) && !empty($event_id) && empty($unique_code) && $_COOKIE['isNew'] == '0')
		{
			...
			foreach ($ticket_types as $key => $ticket) 
			{
				...
				$tableTicketTypeZone->ticket_type_image = $ticket->ticket_type_image;
				$tableTicketTypeZone->total_tickets     = $ticket->total_tickets;
				$tableTicketTypeZone->available_tickets = $ticket->available_tickets;
	   ...
	   //echo "<pre>";print_r($_COOKIE);exit;
		if(isset($_COOKIE['imageID']))
		{
			...
			$uploadedImageCount = count($imageIDs);
	...
	public function saveTicketTypeZone()
	{
		...
		foreach ($ticket_types as $key => $ticket) 
		{
			...
			$tableTicketTypeZone->ticket_zone  = $ticket['ticket_zone'];
			$tableTicketTypeZone->ticket_price = $ticket['ticket_price'];
			//$tableTicketTypeZone->is_deleted   = 1;
			$tableTicketTypeZone->store();

			$html .= '<div id="container'.$tableTicketTypeZone->ticket_type_zone_id.'">
                        <input type="button" id="rmv" class="btn btn-danger" style="float:right;margin-right:1px;" value="Remove">
                        <div class="control-group">
                         <div class="controls1">
                             <input type="text" name="ticket_type[]" id="ticket_type" value="'.$ticket['ticket_type'].'" placeHolder="Enter Ticket Type" required><input type="hidden" id="rgm">
                             <input type="text" name="ticket_zone[]" id="ticket_zone" value="'.$ticket['ticket_zone'].'" placeHolder="Enter Zone" required>
                             <input type="text" name="ticket_price[]" id="ticket_price" value="'.$ticket['ticket_price'].'" placeHolder="Enter Price" id="price_per_ticket" required>
                             <input type="text" name="total_tickets[]" id="total_tickets" value="'.$ticket['total_tickets'].'" disabled>
                			 <input type="text" name="available_tickets[]" id="available_tickets" value="'.$ticket['available_tickets'].'" disabled>
                             <input class="filedata" type="file" id="'.$tableTicketTypeZone->ticket_type_zone_id.'" value="'.$ticket['ticket_type_image'].'" name="filedata[]" />
                             <input type="hidden" name="ticket_type_zone_id[]" value="'.$tableTicketTypeZone->ticket_type_zone_id.'">
                         </div>
                        </div>
                       </div>';
	...
	public function uploadTicketTypeImage()
	{
		JTable::addIncludePath(JPATH_SITE . '/administrator/components/com_beseated/tables');

		$ticket_type_images = $this->input->files->get('file_image','');
		$event_id = $this->input->get('event_id',0);

		//echo "<pre/>";print_r($ticket_type_images);exit;

		$ticket_typeIDs = $this->getTicketTypeIDs($event_id);

		$option = '';

		foreach ($ticket_type_images as $key => $file)
		{
			$tableTicketTypeZone = JTable::getInstance('Eventtickettypezone', 'BeseatedTable');

			static $i1 = 0;
			for ($i = 0; $i < count($ticket_typeIDs); $i++)
			{
				//echo $i1;
				$tableTicketTypeZone->load($ticket_typeIDs[$i1]);
				$i1++;

				if(!strpos($key,'_'))
				{
					$imagePath = $this->uploadTicketTypeImages($event_id,$file);
					...
					{
						$tableTicketTypeZone->ticket_type_image = $imagePath;
					    $tableTicketTypeZone->store();
					}
				}
				
				if($tableTicketTypeZone->ticket_type && $tableTicketTypeZone->ticket_zone && $tableTicketTypeZone->ticket_price)
				{
                    $optionVal = $tableTicketTypeZone->ticket_type.'-'.$tableTicketTypeZone->ticket_zone.'-'.$tableTicketTypeZone->ticket_price;
                }
                elseif($tableTicketTypeZone->ticket_type && $tableTicketTypeZone->ticket_zone && $tableTicketTypeZone->ticket_price == 0)
				{
                    $optionVal = $tableTicketTypeZone->ticket_type.'-'.$tableTicketTypeZone->ticket_zone.'-'.$tableTicketTypeZone->ticket_price;
                }
                elseif($tableTicketTypeZone->ticket_type && $tableTicketTypeZone->ticket_price)
                {
                    $optionVal = $tableTicketTypeZone->ticket_type.'-'.$tableTicketTypeZone->ticket_price;
                }
                elseif($tableTicketTypeZone->ticket_zone && $tableTicketTypeZone->ticket_price)
                {
                    $optionVal = $tableTicketTypeZone->ticket_zone.'-'.$tableTicketTypeZone->ticket_price;
                }
               
				$options.= '<option value="'.$tableTicketTypeZone->ticket_type_zone_id.'">'.$optionVal.'</option>';
				break;
			}
		}
	...
	public function getTicketTypeIDs($event_id)
	{
		// Initialiase variables.
		$db    = JFactory::getDbo();
		$query = $db->getQuery(true);
		...
		$ticketTypeIDs = $db->loadColumn();

		return $ticketTypeIDs;
		
	}

	public function uploadTicketTypeImages($event_id,$file)
	{
		$tmp_name  = $file['tmp_name'];
		$imageName = $file['name'];
		$imageName = str_replace(' ', '', $imageName);
		...
		if(!empty($imageName))
		{
			if ((($file["type"] == "image/gif") || ($file["type"] == "image/jpeg") || ($file["type"] == "image/jpg") || ($file["type"] == "image/png")) && in_array($extension, $allowedExts))
			{
				...
				if(!JFolder::exists(JPATH_ROOT . "images/beseated/Event/ticket_type/"))
				{
					JFolder::create(JPATH_ROOT . "images/beseated/Event/ticket_type/");
				}
				if(!JFolder::exists(JPATH_ROOT . "images/beseated/Event/ticket_type/".$event_id."/"))
				{
					JFolder::create(JPATH_ROOT . "images/beseated/Event/ticket_type/".$event_id."/");
				}
				

				$img = $imgnm . "_" . rand(0, 99999) . "." . $ext;
				$dest = "../images/beseated/Event/ticket_type/$event_id/$img";
				$path = "Event/ticket_type/$event_id/$img";
