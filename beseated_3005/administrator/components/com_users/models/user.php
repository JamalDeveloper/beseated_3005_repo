...
class UsersModelUser extends JModelAdmin
{
	...
	protected function loadFormData()
	{
		...
		if($stdObj->id)
		{
			...
			$userProfile = BeseatedHelper::getBeseatedUserProfile($stdObj->id);

			$beseatedProfile = $this->getBeseatedProfile($stdObj->id,$userProfile);

			if($userProfile->user_type == 'beseated_guest')
			{
				$stdObj->mobile    = $userProfile->phone;
				$stdObj->birthdate = $userProfile->birthdate;
				$stdObj->city      = $userProfile->location;
				$stdObj->only_city = $userProfile->city;
				$stdObj->latitude  = $userProfile->latitude;
				$stdObj->longitude  = $userProfile->longitude;
			}
			else
			{
				$stdObj->city      = $beseatedProfile->location;
				$stdObj->only_city = $beseatedProfile->city;
				$stdObj->latitude  = $beseatedProfile->latitude;
				$stdObj->longitude  = $beseatedProfile->longitude;
			}

			if($userProfile->user_type == 'venue'){
				$stdObj->is_day_club = $beseatedProfile->is_day_club;
			}
	...
	public function getBeseatedProfile($id,$userProfile)
	{
		...
		else
		{
			/*echo "call";
			exit;*/
			/*$query->from($db->quoteName('#__beseated_user_profile'));
			$query->where($db->quoteName('user_id') . ' = ' . $db->quote($id));*/
	...
	public function save($data)
	{
		...
		if(empty($data['only_city']))
		{
			$data['only_city'] = $cityName;
		}
