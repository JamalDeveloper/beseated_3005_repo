...
<head>
   ...
   <!--<link href='<?php echo $this->baseurl; ?>/templates/bcted/css/rangeslider.css' rel='stylesheet' type='text/css' />-->
	...
	<![endif]-->
		...
		/*echo "<pre>";
		print_r($menu->getActive()->link);
		echo "</pre>";*/
		//exit;
		if ($menu->getActive()->link == $menu->getDefault()->link): ?>
			<script src="<?php echo $this->baseurl; ?>/templates/bcted/js/service.ddlist.jquery.js"></script>

		<?php endif; ?>

		<?php if ($menu->getActive()->link == $menu->getDefault()->link): ?>
		  ...
		  jQuery(function () {
		    ...
		    jQuery('#provincesList1').ddlist({
		      ...
		      onSelected: function (index, value, text) {
		        ...
		        console.log(value);
		        document.cookie="search_in_city="+value;
...
jQuery(document).ready(function() {
	...
	jQuery('#fltr-icn').live('click', function(event) {
         jQuery('.filter-bxwrp').toggle('show');
    });
});
</script>
		...
		<div class="container<?php echo ($params->get('fluidContainer') ? '-fluid' : ''); ?>">
			...
			<header class="header" role="banner">
				<div class="header-inner clearfix" id="header-wrapper">
					...
					if(in_array($bctParams->venue, $groups))
					{
						$accessLevel = BeseatedHelper::getGroupAccessLevel($bctParams->venue);
					}
					else if(in_array($bctParams->chauffeur, $groups))
					{
						$accessLevel = BeseatedHelper::getGroupAccessLevel($bctParams->chauffeur);
					}
					else if(in_array($bctParams->protection, $groups))
					{
						$accessLevel = BeseatedHelper::getGroupAccessLevel($bctParams->protection);
					}
					else if(in_array($bctParams->yacht, $groups))
					{
						$accessLevel = BeseatedHelper::getGroupAccessLevel($bctParams->yacht);
					}
					else if(in_array($bctParams->beseated_guest, $groups))
					{
					...
					<div class="logo-menu">
							<a class="brand pull-left" href="<?php echo $homeLink; ?>">
								<?php echo $logo; ?>
                ...
                <?php if(trim($view) == 'userprofile' || trim($view) == 'reset' || trim($view) == 'remind'): ?>
					<div class="background-wrapper ">
				<?php endif; ?>
				<?php if(trim($view) == 'login' || trim($view) == 'registration' || trim($view) == 'packages' || trim($view) == 'clubs' || trim($view) == 'companies' || trim($view) == 'clubbookingdetail'): ?>
					<div class="background-wrapper">
				<?php endif; ?>
			...
			<div class="row-fluid">
				...
				<?php $menu = $app->getMenu(); ?>
				<?php $menuItem = $menu->getItems( 'link', 'index.php?option=com_content&view=article&id=1', true ); ?>
				...
				<main id="content" role="main" class="<?php echo $span; ?>">
					...
					<?php if ($menu->getActive()->link == $menu->getDefault()->link || $view == 'featured'): ?>
					<div class="feature-article-home">
					<?php endif; ?>
						...
						$mymessage = $session->get('beseated_dspl_message');
						$mymessage_title = $session->get('beseated_dspl_title');
						if(!empty($mymessage)){
						?>
							<div id="system-message-container">
								<div id="system-message">
									<div class="alert alert-message">
										...
										<h4 class="alert-heading"><?php if(empty($mymessage_title)){ echo 'Message'; }else{ echo $mymessage_title; } ?></h4>
										<div>
											<p><?php echo $mymessage; $session->clear('beseated_dspl_message'); $session->clear('beseated_dspl_title');?></p>
										</div>
									</div>
								</div>
							</div>
							<?php
						}else{ ?><jdoc:include type="message" /><?php }?>
