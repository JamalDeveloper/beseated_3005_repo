    ...
    <div id="sp-smart-caption-<?php echo $key ?>" class="nivo-html-caption">
        <?php
			...
			if(isset($val['showlink']) and $val['showlink']=='yes' ) echo '<a href="' . $val['link'] . '">';
				...
				{
					// add custom title
					if( isset($val['customtitle']) and !empty($val['customtitle']) ) echo '<p class="nivo-title">' . $val['customtitle'] . '</p>';
				} else {
					// add title
					if( isset($val['title']) and !empty($val['title']) ) echo '<p class="nivo-title">' . $val['title'] . '</p>';  
				}
