...
class beseatedAppHelper
{
	 ...
	 * @since    1.0
	 */
	public function storeNotification($actor,$target,$elementID,$elementType,$notificationType,$title,$cid,$extraParams = array())
	{
		JTable::addIncludePath(JPATH_SITE . '/administrator/components/com_beseated/tables');
	...
	public function getInvitationDetail($bookingID,$bookingType = '')
	{
		...
		if($bookingType == 'event'){
			$query->select('*')
				...
				->where($db->quoteName('event_id') . ' = ' . $db->quote($bookingID))
	...
	public function getBiggestSpender($venue_id)
	{
		...
		{
			if(in_array($spender->user_id, $resLiveUsers) && $spender->user_id != $this->IJUserID)
			{
	 ...
	 * @since    1.0
	 */
	public function uplaodServiceImage($file,$elementType,$elementID)
	{
		$uploadedImage = "";
		$uploadLimit	= 0;
		//$uploadLimit	= ( $uploadLimit * 1024 * 1024 );
		...
		if(is_array($file) && isset($file['size']) && $file['size']>0)
		{
			...
			$storage      = $corePath.$elementType.'/'. $elementID . '/'.$serviceNameType .'/';
			$storageImage = $storage . $hashFileName .  $info['extension'] ;
			$uploadedImage   = $elementType.'/'.$elementID.'/'.$serviceNameType.'/'.$hashFileName.$info['extension'];
				...
				elseif($userGroup == ucfirst('guest'))
				{
					...
					$query->select('COUNT(notification_id) as bookingCount')
						...
						->where('(('.$db->quoteName('notification_type') . ' = ' . $db->quote('venue.paybycash.request.accepted').')'.'OR'.
							    ...
							    '('.$db->quoteName('notification_type') . ' = ' . $db->quote('chauffeur.share.user.paid').'))');
