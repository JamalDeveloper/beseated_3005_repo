 ...
 */
	 ...
	 * @return  void
	 */
	private function outputJSON($jsonarray,$start1 = 0)
	{
		if(!$start1)
		{
		...
		$reqObject = JRequest::getVar('reqObject');
		...
		$model = $this->getModel('ijoomeradv');
		$result1 = $model->getApplicationConfig();

		$usernames = IJOOMER_GC_SEND_EMAIL;

		if(IJOOMER_GC_CREATE_DEBUG == 1)
		{
			$end  = time();
			$diff = $end-$start1;
			$jsonarray['debug']['endTime']   = $end;
			$jsonarray['debug']['difference'] = $diff;
			...
			if(!empty($usernames))
			{
				...
				{
					// Send the email.
					$return = JFactory::getMailer()->sendMail($from, $sender, $mailUser, $subject, $newJsonArray);
			...
			$filePath = getcwd();
			$myFile = $filePath . "/components/com_ijoomeradv/reqObject.txt";
			...
			try
			{
				...
				//$reqData = $message . "\n";
				$reqData = "\n\n================= Request & Response At ".date('Y-m-d H:i')."===========================\n\n Response : ".$message . "\n\n";

				$NewData = $reqData . $existingData ;
					...
					foreach ($puserlist as $puser)
					{
						...
						if ($configallow && $puser->userid != $this->IJUserID && !empty($puser))
						{
							if (IJOOMER_PUSH_ENABLE_IPHONE == 1 && $puser->device_type == 'iphone')
							{
								...
								$options['aps']['alert']       = strip_tags($jsonarray['pushNotificationData']['message']);
								$options['aps']['type']        = $jsonarray['pushNotificationData']['type'];
								$options['aps']['userid']      = $puser->userid;
								$options['aps']['id']          = ($jsonarray['pushNotificationData']['id'] != 0) ? $jsonarray['pushNotificationData']['id'] : $jsonarray['pushNotificationData']['multiid'][$puser->userid];
								$options['aps']['elementType'] = (!empty($jsonarray['pushNotificationData']['elementType'])) ? $jsonarray['pushNotificationData']['elementType'] : '';
							...
							if (IJOOMER_PUSH_ENABLE_ANDROID == 1 && $puser->device_type == 'android')
							{
								...
								$options['data']['message']     = strip_tags($jsonarray['pushNotificationData']['message']);
								$options['data']['type']        = $jsonarray['pushNotificationData']['type'];
								$options['data']['userid']      = $puser->userid;
								$options['data']['id']          = ($jsonarray['pushNotificationData']['id'] != 0) ? $jsonarray['pushNotificationData']['id'] : $jsonarray['pushNotificationData']['multiid'][$puser->userid];
								$options['data']['elementType'] = (!empty($jsonarray['pushNotificationData']['elementType'])) ? $jsonarray['pushNotificationData']['elementType'] : '';
								//$options['data']['id'] = ($jsonarray['pushNotificationData']['id'] != 0) ? $jsonarray['pushNotificationData']['id'] : $jsonarray['pushNotificationData']['multiid'][$puser->userid];
								IJPushNotif::sendAndroidPushNotification($options,$puser->user_type);
							}
						}
					}
				}
	...
	public function display($cachable = false, $urlparams = array())
	{
		...
		if(IJOOMER_GC_CREATE_DEBUG == '1')
		{
			$jsonarray['debug']['startTime'] = $start;
		}

		// Send data array to create jason string and output
		$this->outputJSON($jsonarray,$start);
	}
	 ...
	 *            "lat":"23.00",
	 *            "long":"72.40",
	 *            "devicetoken":"android_devicetoken",
	 *            "type":"android/iphone"
	 *            "loginType":"Guest/Company"
	 *        }
	 *    }
	 *
	 * @return void
	...
	public function login()
	{
		...
		$fb = IJReq::getTaskData('fb',0);
		$loginType = IJReq::getTaskData('loginType','','string');

		if (!IJReq::getTaskData('email') or !IJReq::getTaskData('password'))
		{
			// Check if email or password not blank
			$jsonarray['code'] = 400;
		...
		{
			$jsonarray['code'] = 401;
			$jsonarray['message'] = JText::_('COM_IJOOMERADV_UNDEFINE_LOGIN_TYPE');
		...
		{
			$jsonarray['code'] = 402;
			$jsonarray['message'] = JText::_('COM_IJOOMERADV_ACCOUNT_NOT_ACTIVATED');
		...
		$foundUser      = JFactory::getUSer($userDetail->id);
		$beseatedParams = BeseatedHelper::getExtensionParam();
		$groups         = $foundUser->get('groups');
		$companyType    = array('Chauffeur','Protection','Venue','Yacht');
		...
		if($loginType != $userType && $loginType != 'Company')
		{
			$jsonarray['code'] = 401;
			$jsonarray['message'] = JText::_('COM_IJOOMERADV_UNDEFINE_LOGIN_TYPE');
		...
		else if($loginType == "Company" && !in_array($userType, $companyType))
		{
			$jsonarray['code'] = 401;
			$jsonarray['message'] = JText::_('COM_IJOOMERADV_UNDEFINE_LOGIN_TYPE');
		...
		$querySelForgot->select('token')
			...
			->where($db->quoteName('user_id') . ' = ' . $db->quote($userDetail->id));

		// Set the query and load the result.
		$db->setQuery($querySelForgot);
		$tempPassword = $db->loadResult();
		/*********************** -- End of Forgot Password Reset -- **************************/

		$credentials = array();

		// Get username
		$credentials['username'] = IJReq::getTaskData('email');
		...
		if($fb == 1)
		{
			...
			$Lresult = $this->mainframe->triggerEvent ( 'onUserLogin', array (( array ) $response, $options ) );
	...
	/**
	 * @uses this function is used to forgot password
	 * @example the json string will be like, :
	 * 	{"task":"forgotPassword","taskData":{"email":"email"}}
	 *
	 */
	function forgotPassword(){
		$model = $this->getModel('ijoomeradv');
		//$jsonarray = $model->forgotPassword();
		$jsonarray = $model->forgotPassword();
	 ...
	 * @example the json string will be like, :
	 * {"task":"registration","taskData":{"name":"Full name","city":"City name","birthdate":"Date of Birth (yyyy-mm-dd)","mobile":"mobile","password":"password","email":"email"}}
	 *
	 * @return void
	 */
