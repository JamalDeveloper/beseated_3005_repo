...
class IjoomeradvHelper
{
	...
	public function getDebug_config()
	{
		$query = $this->db->getQuery(true);
		...
		$query->select('name, value')
			->from($this->db->qn('#__ijoomeradv_config'))
			->where($this->db->qn('name') . ' = ' . $this->db->q('IJOOMER_GC_CREATE_DEBUG'));
	...
	public function getRequestedObject()
	{
		...
		if (JRequest::get('post'))
		{
			...
			$this->mainframe->IJObject = new stdClass;
			if ($encryption == 1)
			{
			...
			else
			{
				...
				if ($debug->value == 1 )
				{
					...
					$message = JRequest::getVar('reqObject');
...
class IJReq
{
	...
	{
		$mainframe                = JFactory::getApplication();
		$ijObject                 = new stdClass;
		$ijObject                 = $mainframe->IJObject;
		$ijObject->response       = new stdClass;
		$ijObject->response->code = intval($default);

		$mainframe->IJObject = $ijObject;
	}
...
class IJPushNotif
{
	...
	public static function sendPushNotification($jsonarray)
	{
		...
		if($memberlist)
		{
			...
			foreach ($puserlist as $puser)
			{
				...
				if ($configallow && $puser->userid != $user->id && !empty($puser))
				{
	 ...
	 * @return  array
	 */
	public static function sendIphonePushNotification($options,$userType)
	{
		require_once JPATH_SITE . '/components/com_ijoomeradv/extensions/beseated/helper.php';

		$helper            = new beseatedAppHelper;

		$allNotificationCount =  $helper->getBangeCount($options['aps']['userid']);
		...
		$body['aps']['badge'] = ($allNotificationCount) ? $allNotificationCount:0;

		//echo "<pre/>";print_r($bo);exit;
	 ...
	 * @return  void
	 */
	public static function sendAndroidPushNotification($options,$userType)
	{
		require_once JPATH_SITE . '/components/com_ijoomeradv/extensions/beseated/helper.php';
		...
		$allNotificationCount =  $helper->getBangeCount($options['data']['userid']);

		$url = 'https://android.googleapis.com/gcm/send';
		//$options['data']['badge'] = (isset($options['data']['badge']) && !empty($options['data']['badge'])) ? $options['data']['badge'] : 1;
		$options['data']['badge'] = ($allNotificationCount) ? (string)$allNotificationCount:"0";
