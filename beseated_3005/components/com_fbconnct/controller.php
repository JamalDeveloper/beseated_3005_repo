...
class fbconnctController extends JControllerLegacy
{
		...
		if(fbconnctController::count_jemail($uemail))
		{
			...
			if(!fbconnctController::count_j_fb_user($j_uid ))
			{
				if(count($onlyFBids))
				{
					...
					$FBFriends = implode(",", $onlyFBids);


				}

				$fbinsertquary="INSERT INTO #__facebook_joomla_connect(joomla_userid,facebook_userid,facebook_friends,joined_date,linked) VALUES ($j_uid,$uid,'".$FBFriends."',$intdatetime,1)";
			...
			if(count($onlyFBids))
			{
				...
				$query->update($db->quoteName('#__facebook_joomla_connect'))
					->set($db->quoteName('facebook_friends') . ' = ' . $db->quote($FBFriends))
					->where($db->quoteName('joomla_userid') . ' = ' . $db->quote($j_uid))
					->where($db->quoteName('facebook_userid') . ' = ' . $db->quote($uid));
				...
				$query->update($db->quoteName('#__beseated_user_profile'))
					->set($db->quoteName('fb_friends_id') . ' = ' . $db->quote($FBFriends))
					->set($db->quoteName('fb_id') . ' = ' . $db->quote($uid))
					->set($db->quoteName('is_fb_user') . ' = ' . $db->quote('1'))
					->where($db->quoteName('is_deleted') . ' = ' . $db->quote('0'))
					->where($db->quoteName('user_id') . ' = ' . $db->quote($j_uid));
			...
			$redirAfterLogin = JRequest::getVar('return');

			require_once JPATH_SITE . '/components/com_beseated/helpers/beseated.php';
			$profileMenu = BeseatedHelper::getBeseatedMenuItem('userprofile-bctguest');

			if(empty($redirAfterLogin))
			{
				//$redirAfterLogin= $myparams->get('redirect-after-login');
				$redirAfterLogin = JURI::base().'index.php?option=com_beseated&view=userprofile&task=fb_login&Itemid='.$profileMenu->id;
				//echo $redirAfterLogin;exit();
			}
			else
			{
				$redirAfterLogin = base64_decode ($redirAfterLogin);
				$redirAfterLogin = JURI::base().$redirAfterLogin;
			}
	...
	public static function create_proceed()
	{
		...
		require_once JPATH_SITE . '/components/com_beseated/helpers/beseated.php';
		$profileMenu = BeseatedHelper::getBeseatedMenuItem('userprofile-bctguest');
        $redirAfterReg = JURI::base().'index.php?option=com_beseated&view=userprofile&task=fb_login&Itemid='.$profileMenu->id;
		...
		$newUsertype = $usersConfig->get( 'new_usertype', 14);
		...
		$userData['fbid']       = $uid;
		$userData['first_name'] = $session_me['first_name'];
		$userData['last_name']  = $session_me['last_name'];
		...
		if ($user->save())
		{
			...
			if(count($onlyFBids))
			{
				$storeFriendsIds = implode(",", $onlyFBids);
			}
			else
			{
				$storeFriendsIds = '';
			...
			{
				$fbinsertquary="UPDATE #__facebook_joomla_connect SET joomla_userid=$jomuserid,facebook_friends='$storeFriendsIds',joined_date=$intdatetime WHERE facebook_userid=$uid";
			}
			else
			{
				$fbinsertquary="INSERT INTO #__facebook_joomla_connect(joomla_userid,facebook_userid,facebook_friends,joined_date,linked) VALUES ($jomuserid,$uid,'$storeFriendsIds',$intdatetime,1)";
			}
			$db->setQuery($fbinsertquary);
			$result = $db->query();
			...
			// Create the base update statement.
			$query->update($db->quoteName('#__beseated_user_profile'))
				->set($db->quoteName('fb_friends_id') . ' = ' . $db->quote($storeFriendsIds))
				->set($db->quoteName('fb_id') . ' = ' . $db->quote($uid))
				->set($db->quoteName('is_fb_user') . ' = ' . $db->quote('1'))
				->where($db->quoteName('is_deleted') . ' = ' . $db->quote('0'))
				->where($db->quoteName('user_id') . ' = ' . $db->quote($jomuserid));
	...
	public static function try_connect()
	{
		...
		if (!$fbuser){
			...
			if(!$sef_enabled){
				$fb_redirect_url = JURI::base().'index.php?option=com_fbconnct&task=login&view=fbconnct&format=raw&return='.$redirAfterLogin;
			}else{
				$fb_redirect_url = JURI::base().substr(JRoute::_('index.php?option=com_fbconnct&task=login&view=fbconnct&format=raw&return='.$redirAfterLogin), strlen(JURI::base(true)) + 1);
			}
		...
		$fbdata = array('fbid'=>$fbuser,'fbdetails'=>$me,'appid'=>$getappid, 'fbtoken'=>$access_token,'fbobject'=>$facebook,'fbfriends'=>$friends);
		return $fbdata;
	}
