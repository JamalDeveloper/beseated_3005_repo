...
function add_to_blacklist(userID,protectionID)
{
    jQuery.ajax({
        url: 'index.php?option=com_beseated&task=protectionaccounthistory.add_user_in_blacklist',
        type: 'GET',
        data: '&user_id='+userID+'&protection_id='+protectionID,
...
function remove_from_blacklist(userID,protectionID)
{

    jQuery.ajax({
        url: 'index.php?option=com_beseated&task=protectionaccounthistory.remove_user_from_blacklist',
        type: 'GET',
        data: '&user_id='+userID+'&protection_id='+protectionID,
...
<?php
	$blackListedUsers = BeseatedHelper::getBlackList($this->elementDetail->protection_id,'Protection');
		...
		<table class="activity" id="accordion">
			...
			<tbody>
				...
				<?php foreach ($this->history as $key => $value):?>
					...
					<?php if(in_array($value->protection_booking_id, $processIDs)){ continue; } ?>
					<?php $processIDs[] = $value->protection_booking_id; ?>
					<tr id="row<?php echo $value->protection_booking_id; ?>" class="accordion" data-toggle="collapse" data-target="#demo<?php echo $value->protection_booking_id; ?>">
					...
					<tr class="expand-data">
						<td class="hiddenRow" colspan="3">
							<div class="collapse" id="demo<?php echo $value->protection_booking_id; ?>">
								<table class="tbl-hdndata">
									<tbody>
										<tr>
											<td colspan="3">
												<?php if(in_array($value->user_id, $blackListedUsers)): ?>
													<center><button onclick="remove_from_blacklist('<?php echo $value->user_id; ?>','<?php echo $this->elementDetail->protection_id; ?>')" class="btn btn-primary">Remove from blacklist</button></center>
												<?php else: ?>
													<center><button onclick="add_to_blacklist('<?php echo $value->user_id; ?>','<?php echo $this->elementDetail->protection_id; ?>')" class="btn btn-primary">Blacklist User</button></center>
												<?php endif; ?>
											</td>
											<td rowspan="2" valign="bottom">
												...
												<?php if($isLiveUser): ?>
													...
													<?php $menuItem = $menu->getItems( 'link', 'index.php?option=com_beseated&view=protectionmessages', true ); ?>
													<?php $Itemid = $menuItem->id; ?>
													<?php $connectionID = BeseatedHelper::getMessageConnection($this->elementDetail->user_id,$value->user_id); ?>
													<?php $link = JRoute::_("index.php?option=com_beseated&view=protectionmessagedetail&user_id=".$value->user_id."&connection_id=".$connectionID."&Itemid=".$Itemid); ?>
