...
/**
 * The Beseated Club Friends Attending View
 *
 * @since  0.0.1
 */
class BeseatedViewClubFriendsAttending extends JViewLegacy
{
	protected $items;
	...
	protected $model;
	/**
	 * Display the Beseated Club Friends Attending view
	 *
	...
	function display($tpl = null)
	{
		...
		if(!$this->user->id)
        {
			...
			$property    = array($accessLevel,'index.php?option=com_beseated&view=clubfriendsattending');
			$menuItem2   = $menu->getItems( $access, $property, true );
			$link2       = 'index.php?option=com_beseated&view=clubfriendsattending&club_id='.$this->clubID.'&Itemid='.$menuItem2->id;
		...
		$this->items         = $this->get('MyFbFriendID');
