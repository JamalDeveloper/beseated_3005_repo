...
//$document->addScript(JUri::root().'components/com_beseated/assets/carousal/jquery.js');
...
<div class="venuedetail clb-infowrp">
	<div class="venue-img">
	            		...
	            		<ul>
							<?php for ($i = 0; $i < $imagesCount; $i++):?>
								<?php if($this->images[$i]->is_video > 0):?>
									<li>
									<video id="display_venue_images" width="600" height="400"  controls>
										<source src="<?php echo JUri::base()."images/beseated/".$this->images[$i]->image; ?>" type="video/mp4">
										Your browser does not support HTML5 video.
									</video>
									</li>
									<?php elseif(!empty($this->images[$i]->image)):?>
				        					<li><img src="<?php echo JUri::base()."images/beseated/".$this->images[$i]->image;?>" width="600" height="400" alt=""></li>
									<?php else: ?>
		...
		<?php elseif($imagesCount == 1): ?>
			<?php if($this->images[0]->is_video > 0): die;?>
			<video width="700" heght="460" controls>
			...
			<?php elseif(!empty($this->images[0]->image)):?>
				<img src="<?php echo JUri::base().'images/beseated/'.$this->images[0]->image; ?>" alt="" />
			<?php else: ?>
				<img src="<?php echo JUri::base()."images/beseated/default/banner.png";?>" alt="" />
			<?php endif; ?>
		<?php endif; ?>
		...
		<div class="span12 bigspenders">
			<?php $biggestSpenders      = BeseatedHelper::getBiggestSpender($this->club->venue_id);
				  $biggestSpendersCount = count($biggestSpenders);?>
			<?php if ($biggestSpendersCount > 0): ?>
				<span>Biggest Spenders</span>
				<div class="inner-bigspenders">
					<ul>
					<?php for ($j = 0; $j < $biggestSpendersCount; $j++): ?>
						<?php if (!empty($biggestSpenders[$j]['thumbAvatar'])):?>
							<li>
								<img src="<?php echo $biggestSpenders[$j]['thumbAvatar'];?>" alt="" />
							</li>
						<?php endif; ?>
					<?php endfor;?>
					</ul>
