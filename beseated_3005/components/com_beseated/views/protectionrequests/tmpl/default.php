				...
				<ul>
					...
					<?php if($this->bookings): ?>
						...
						<?php foreach ($this->bookings as $key => $booking):?>
							<li id="request_booking_<?php echo $booking->protection_booking_id; ?>">
								<div class="main-request-booking">
									...
									<div class="request-booking-detail" onclick= "expandDetail(<?php echo $booking->protection_booking_id;?>)">
										...
										<div class="request-booking-location location-<?php echo $booking->protection_booking_id;?>" style="display:none">
										...
										<div class="request-booking-capacity">
											<?php echo $booking->total_guard.' Bodyguard(s)';?>
										</div>
									...
									<div class="status">
										<?php if($booking->protection_status == 3): ?>
											<input type="button" class="set-status-btn" onclick="deleteBookingRequest(<?php echo $booking->protection_booking_id;?>,'1')" value="Awaiting Confirmation">
										<?php elseif($booking->protection_status	 == 1): ?>
											<input type="button" class="set-status-btn decline" value="Accept" onclick="changeBookingStatus(<?php echo $booking->protection_booking_id;?>,'3')">
											<input type="button" class="set-status-btn decline" value="Decline" onclick="changeBookingStatus(<?php echo $booking->protection_booking_id;?>,'6')">
										<?php elseif($booking->protection_status	 == 6): ?>
											<input type="button" class="set-status-btn" value="Delete" onclick="deleteBookingRequest(<?php echo $booking->protection_booking_id;?>,'0')">
										<?php endif; ?>
									</div>
								</div>
							</li>
...
<div id="declineBookingModal"  class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header">Decline Request</div>
...
function deleteBookingRequest(protectionBookingID,type)
{
	...
	jQuery('.delete-request').click(function (e) 
	{
		jQuery.ajax({
			type: "GET",
			url: "index.php?option=com_beseated&task=protectionrequests.deleteBookingRequest",
			data: "protectionBookingID="+protectionBookingID,
...
function changeBookingStatus(protectionBookingID,statusCode)
{
	...
	if(parseInt(statusCode) == 6)
	{
		...
		jQuery('.decline-request').click(function (e) 
		{
			jQuery.ajax({
				type: "GET",
				url: "index.php?option=com_beseated&task=protectionrequests.changeBookingStatus",
				data: "&protectionBookingID="+protectionBookingID+"&statusCode=6",
				success: function(response){
					if(response == "200")
					{
						location.reload();
	...
	else
	{
		jQuery.ajax({
				type: "GET",
				url: "index.php?option=com_beseated&task=protectionrequests.changeBookingStatus",
				data: "&protectionBookingID="+protectionBookingID+"&statusCode=3",
				success: function(response){
					if(response == "200")
					{
						location.reload();
