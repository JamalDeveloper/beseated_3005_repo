...
function add_to_blacklist(userID,yachtID)
{
    jQuery.ajax({
        url: 'index.php?option=com_beseated&task=yachtaccounthistory.add_user_in_blacklist',
        type: 'GET',
        data: '&user_id='+userID+'&yacht_id='+yachtID,
...
function remove_from_blacklist(userID,yachtID)
{

    jQuery.ajax({
        url: 'index.php?option=com_beseated&task=yachtaccounthistory.remove_user_from_blacklist',
        type: 'GET',
        data: '&user_id='+userID+'&yacht_id='+yachtID,
...
<?php
	$blackListedUsers = BeseatedHelper::getBlackList($this->elementDetail->yacht_id,'Yacht');
		...
		<table class="activity" id="accordion">
			...
			<tbody>
				...
				<?php foreach ($this->history as $key => $value):?>
					...
					<?php if(in_array($value->yacht_booking_id, $processIDs)){ continue; } ?>
					<?php $processIDs[] = $value->yacht_booking_id; ?>
					<tr id="row<?php echo $value->yacht_booking_id; ?>" class="accordion" data-toggle="collapse" data-target="#demo<?php echo $value->yacht_booking_id; ?>">
					...
					<tr class="expand-data">
						<td class="hiddenRow" colspan="3">
							<div class="collapse" id="demo<?php echo $value->yacht_booking_id; ?>">
								<table class="tbl-hdndata">
									<tbody>
										<tr>
											<td colspan="3">
												<?php if(in_array($value->user_id, $blackListedUsers)): ?>
													<center><button onclick="remove_from_blacklist('<?php echo $value->user_id; ?>','<?php echo $this->elementDetail->yacht_id; ?>')" class="btn btn-primary">Remove from blacklist</button></center>
												<?php else: ?>
													<center><button onclick="add_to_blacklist('<?php echo $value->user_id; ?>','<?php echo $this->elementDetail->yacht_id; ?>')" class="btn btn-primary">Blacklist User</button></center>
												<?php endif; ?>
											</td>
											<td rowspan="2" valign="bottom">
												...
												<?php if($isLiveUser): ?>
													...
													<?php $menuItem = $menu->getItems( 'link', 'index.php?option=com_beseated&view=yachtmessages', true ); ?>
													<?php $Itemid = $menuItem->id; ?>
													<?php $connectionID = BeseatedHelper::getMessageConnection($this->elementDetail->user_id,$value->user_id); ?>
													<?php $link = JRoute::_("index.php?option=com_beseated&view=yachtmessagedetail&user_id=".$value->user_id."&connection_id=".$connectionID."&Itemid=".$Itemid); ?>
