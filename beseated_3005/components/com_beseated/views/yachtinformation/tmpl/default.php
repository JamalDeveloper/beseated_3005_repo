...
$link2               = 'index.php?option=com_beseated&view=yachtinformation&yacht_id='.$yachtId.'&Itemid='.$menuItem2->id;
...
<div class="chauffer-wrapper">
	<div class="chauffeurs-title">
		<?php echo ucfirst($yachtDetails->yacht_name); ?>
	</div>
	...
	<div class="chauffeurs-fav">
		<div class="favourite-wrp">
			<button id="favourite-add" onclick="addYachtToFavourite('<?php echo $yachtDetails->yacht_id; ?>','<?php echo $this->user->id; ?>')" type="button" class="fav-btn"></button>
			<button id="favourite-remove" onclick="removeYachtFromFavourite('<?php echo $yachtDetails->yacht_id; ?>','<?php echo $this->user->id; ?>')" type="button" class="fav-btn active"></button>
...
<script type="text/javascript">
	function addYachtToFavourite(yachtID,userID)
	{
		jQuery.ajax({
			url: 'index.php?option=com_beseated&task=yachts.addtofavourite',
			type: 'GET',
			data: '&yacht_id='+yachtID+'&user_id='+userID,
	...
	function removeYachtFromFavourite(yachtID,userID)
	{
		jQuery.ajax({
			url: 'index.php?option=com_beseated&task=yachts.removefromfavourite',
			type: 'GET',
			data: '&yacht_id='+yachtID+'&user_id='+userID,
			success: function(response){
	        	if(response == "1" || response == "2")
