...
$menuItem            = $menu->getItems( 'link', 'index.php?option=com_beseated&view=chauffeurs', true );
...
<div class="wrapper">
	<button class="filter-btn" type='button'  id='fltr-icn' value='hide/show'></button>
		<form action="<?php echo JRoute::_('index.php?option=com_beseated&view=chauffeurs&Itemid='.$Itemid);?>" name="adminForm" method="post" class="form-inline" id="search_form">
			...
			<div class="control-group flter-option">
					<div class="controls">
						<div class="control-group">
							...
							<?php endif; ?>
							<label for="c2"><?php echo JText::_('COM_BESEATED_CHAUFFEURS_FILTER_CHAUFFEURS_NEAR_ME'); ?></label>
						</div>
					</div>
			</div>
			<div class="control-group">
   	...
   	<div class="span12 filter-result">
			<?php $menuItem = $menu->getItems( 'link', 'index.php?option=com_beseated&view=chauffeurinformation', true );?>
			<?php foreach ($this->items as $key => $result):?>
				<?php $hasDisplay = 1;?>
				<div class="span4 venue_blck ">
					<div class="venue-img">
						<?php if(!empty($result->thumb_image)): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_beseated&view=chauffeurinformation&chauffeur_id='.$result->chauffeur_id.'&Itemid='.$menuItem->id ) ?>"><img src="<?php echo JUri::base().'images/beseated/'. $result->thumb_image; ?>" alt="" /></a>
						<?php elseif(!empty($result->image)): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_beseated&view=chauffeurinformation&chauffeur_id='.$result->chauffeur_id.'&Itemid='.$menuItem->id ) ?>"><img src="<?php echo JUri::base().'images/beseated/'. $result->image; ?>" alt="" /></a>
						<?php else: ?>
							<a href="<?php echo JRoute::_('index.php?option=com_beseated&view=chauffeurinformation&chauffeur_id='.$result->chauffeur_id.'&Itemid='.$menuItem->id) ?>"><img src="images/bcted/default/banner.png" alt="" /></a>
						<?php endif; ?>
