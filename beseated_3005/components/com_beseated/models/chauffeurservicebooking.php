 ...
 * @since  0.0.1
 */
class BeseatedModelChauffeurServiceBooking extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function bookChauffeurService($data = array())
	{
	...
	public function getServiceDetail()
	{
		...
		$query->select('s.*')
			->from($db->quoteName('#__beseated_chauffeur_services', 's'))
			->where($db->quoteName('s.published') . ' = ' . $db->quote('1'))
			->where($db->quoteName('s.service_id') . ' = ' . $db->quote($serviceID));
		$query->select('c.*')
     		->join('LEFT','#__beseated_chauffeur AS c ON s.chauffeur_id=c.chauffeur_id');
