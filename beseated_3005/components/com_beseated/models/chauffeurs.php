...
class BeseatedModelChauffeurs extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select("c.*,img.thumb_image,img.image,(((acos(sin((".$latitude."*pi()/180)) *
					            sin((c.latitude*pi()/180))+cos((".$latitude."*pi()/180)) *
					            cos((c.latitude*pi()/180)) * cos(((".$longitude."- c.longitude)*
			...
			->where($db->quoteName('c.has_service') . ' = ' . $db->quote('1'))
			...
			->group($db->quoteName('c.chauffeur_id'));
		...
		{
			$query->where('('.$db->quoteName('c.city') . ' LIKE  ' .  $db->quote('%'.$search_in_city.'%').' OR '.$db->quoteName('c.location') . ' LIKE  ' .  $db->quote('%'.$search_in_city.'%').')');
		}
		...
		if($inner_search == 1)
		{
			if(!empty($caption))
			{
				$query->where($db->quoteName('c.chauffeur_name') . ' LIKE  ' .  $db->quote('%'.$caption.'%'));
			}
			...
			{
				$query->order($db->quoteName('c.chauffeur_name') . ' ASC');
			}
		}
	 ...
	 * @since   1.6.1
	 */
	public function addtofavourite($ChauffeurID, $userID)
	{
		if($this->checkForAlreadyFavourite($ChauffeurID,'Chauffeur',$userID))
		{
			return 2;
		}
		...
		$tblFavourite->load(0);
		$data                 = array();
		$data['element_type'] = 'Chauffeur';
		$data['element_id']   = $ChauffeurID;
		$data['user_id']      = $userID;
	...
	public function removefromfavourite($ChauffeurID, $userID)
	{
