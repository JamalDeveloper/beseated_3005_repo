 ...
 * @since  0.0.1
 */
class BeseatedModelChauffeurAccountHistory extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if($elementType != 'Chauffeur')
		{
			return array();
		}

		if(!$elementDetail->chauffeur_id)
		{
			return array();
		}
		...
		$query->select('yb.*')
			->from($db->quoteName('#__beseated_chauffeur_booking','yb'))
			->where($db->quoteName('yb.chauffeur_id') . ' = ' . $db->quote($elementDetail->chauffeur_id))
			->where($db->quoteName('yb.chauffeur_status') . ' IN ('.implode(',', $bookingStatus).')')
			->where($db->quoteName('yb.booking_date')  . ' < ' . $db->quote(date('Y-m-d')))
			->where($db->quoteName('yb.deleted_by_chauffeur') . ' = ' . $db->quote(0))
		...
		$query->select('ys.service_name,ys.thumb_image,ys.image')
			->join('LEFT','#__beseated_chauffeur_services AS ys ON ys.service_id=yb.service_id');

		return $query;
	}

	public function addUserToBlackList($userID,$chauffeurID,$elementType = 'Chauffeur')
	{
		$isBlackListed = BeseatedHelper::checkBlackList($userID,$chauffeurID,$elementType);
		if($isBlackListed)
		{
			return "706";
		}

		$isAdded = BeseatedHelper::addUserToBlackList($userID,$chauffeurID,$elementType);
		if($isAdded)
		{
			...
			$query->delete()
				...
				->where($db->quoteName('element_id') . ' = ' . $db->quote((int) $chauffeurID));
	...
	public function removeUserFromBlackList($userID,$chauffeurID,$elementType = 'Chauffeur')
	{
		BeseatedHelper::removeUserFromBlackList($userID,$chauffeurID,$elementType);
