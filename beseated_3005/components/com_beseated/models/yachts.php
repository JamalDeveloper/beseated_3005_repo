...
class BeseatedModelYachts extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select("y.*,img.thumb_image,img.image,(((acos(sin((".$latitude."*pi()/180)) *
					            sin((y.latitude*pi()/180))+cos((".$latitude."*pi()/180)) *
					            cos((y.latitude*pi()/180)) * cos(((".$longitude."- y.longitude)*
			...
			->where($db->quoteName('y.has_service') . ' = ' . $db->quote('1'))
			->where($db->quoteName('u.block') . ' = ' . $db->quote('0'))
			->join('LEFT','#__beseated_element_images AS img ON img.element_id=y.yacht_id')
			->join('INNER','#__users AS u ON u.id=y.user_id')
			->group($db->quoteName('y.yacht_id'));
		...
		{
			$query->where('('.$db->quoteName('y.city') . ' LIKE  ' .  $db->quote('%'.$search_in_city.'%').' OR '.$db->quoteName('y.location') . ' LIKE  ' .  $db->quote('%'.$search_in_city.'%').')');
		}
	    ...
	    if($inner_search == 1)
		{
			if(!empty($caption))
			{
				$query->where($db->quoteName('y.yacht_name') . ' LIKE  ' .  $db->quote('%'.$caption.'%'));
			}
			...
			{
				$query->order($db->quoteName('y.yacht_name') . ' ASC');
			}
		}
	 ...
	 * @since   1.6.1
	 */
	public function addtofavourite($yachtID, $userID)
	{
		if($this->checkForAlreadyFavourite($yachtID,'Yacht',$userID))
		{
			return 2;
		}
		...
		$tblFavourite->load(0);
		$data                 = array();
		$data['element_type'] = 'Yacht';
		$data['element_id']   = $yachtID;
		$data['user_id']      = $userID;
	...
	public function removefromfavourite($YachtID, $userID)
	{
