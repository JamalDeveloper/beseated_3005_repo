 ...
 * @since  0.0.1
 */
class BeseatedModelChauffeurrequestpay extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getChauffeurBookingDetail()
	{
		...
		$chauffeur_booking_id = $input->getInt('chauffeur_booking_id',0);

		$db    = JFactory::getDbo();

		$query = $db->getQuery(true);
			// Create the base select statement.
			$query->select('yb.chauffeur_booking_id,yb.chauffeur_id,yb.booking_date,yb.pickup_location,yb.dropoff_location,yb.booking_time,yb.user_id,yb.capacity,yb.total_price,yb.user_status,yb.booking_currency_code,yb.booking_currency_sign,yb.is_splitted,yb.has_invitation,yb.each_person_pay,yb.splitted_count,yb.remaining_amount,yb.response_date_time,yb.total_split_count')
				->from($db->quoteName('#__beseated_chauffeur_booking') . ' AS yb')
				->where($db->quoteName('yb.chauffeur_booking_id') . ' = ' . $db->quote($chauffeur_booking_id));

			$query->select('y.chauffeur_name,y.location,y.city,y.deposit_per,y.refund_policy')
				->join('LEFT','#__beseated_chauffeur AS y ON y.chauffeur_id=yb.chauffeur_id');

			$query->select('ys.service_name,ys.service_type,ys.thumb_image,ys.image')
