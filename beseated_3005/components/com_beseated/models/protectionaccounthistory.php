 ...
 * @since  0.0.1
 */
class BeseatedModelProtectionAccountHistory extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select('yb.*')
			->from($db->quoteName('#__beseated_protection_booking','yb'))
			->where($db->quoteName('yb.protection_id') . ' = ' . $db->quote($elementDetail->protection_id))
			->where($db->quoteName('yb.protection_status') . ' IN ('.implode(',', $bookingStatus).')')
			->where($db->quoteName('yb.booking_date')  . ' < ' . $db->quote(date('Y-m-d')))
			->where($db->quoteName('yb.deleted_by_protection') . ' = ' . $db->quote(0))
		...
		$query->select('ys.service_name,ys.thumb_image,ys.image')
			->join('LEFT','#__beseated_protection_services AS ys ON ys.service_id=yb.service_id');

		return $query;
	}

	public function addUserToBlackList($userID,$protectionID,$elementType = 'Protection')
	{
		$isBlackListed = BeseatedHelper::checkBlackList($userID,$protectionID,$elementType);
		if($isBlackListed)
		{
			return "706";
		}

		$isAdded = BeseatedHelper::addUserToBlackList($userID,$protectionID,$elementType);
		if($isAdded)
		{
			...
			$query->delete()
				...
				->where($db->quoteName('element_id') . ' = ' . $db->quote((int) $protectionID));
	...
	public function removeUserFromBlackList($userID,$protectionID,$elementType = 'Protection')
	{
		BeseatedHelper::removeUserFromBlackList($userID,$protectionID,$elementType);
