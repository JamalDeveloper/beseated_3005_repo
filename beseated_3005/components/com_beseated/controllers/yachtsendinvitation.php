 ...
 * @since  0.0.1
 */
class BeseatedControllerYachtsendinvitation extends JControllerAdmin
{
	/**
	 * Constructor.
	 *
	 ...
	 * @since   0.0.1
	 */
	public function getModel($name = 'YachtSendInvitation', $prefix = 'BeseatedModel', $config = array('ignore_request' => true))
	{
	...
	function sendInvitation()
	{
		...
		if($inviteType == 'email')
		{
			...
			{ 
				...
				{
				    $link = JURI::root().'index.php?option=com_beseated&view=yachtsendinvitation&booking_id='.$bookingID.'&Itemid='.$Itemid;
		...
		$alreadyInvited       = BeseatedHelper::getInvitationDetail($bookingID,'yacht');
		$alreadySplitted      = BeseatedHelper::getBookedElementShareInvitations('yacht',$bookingID);
		...
		{
			$link = JURI::root().'index.php?option=com_beseated&view=yachtinviteduserstatus&booking_id='.$bookingID.'&Itemid='.$Itemid;
