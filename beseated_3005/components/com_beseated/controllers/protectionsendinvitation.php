 ...
 * @since  0.0.1
 */
class BeseatedControllerProtectionsendinvitation extends JControllerAdmin
{
	/**
	 * Constructor.
	 *
	 ...
	 * @since   0.0.1
	 */
	public function getModel($name = 'ProtectionSendInvitation', $prefix = 'BeseatedModel', $config = array('ignore_request' => true))
	{
	...
	function sendInvitation()
	{
		...
		if($inviteType == 'email')
		{
			...
			{ 
				...
				{
				    $link = JURI::root().'index.php?option=com_beseated&view=protectionsendinvitation&booking_id='.$bookingID.'&Itemid='.$Itemid;
		...
		$alreadyInvited       = BeseatedHelper::getInvitationDetail($bookingID,'protection');
		$alreadySplitted      = BeseatedHelper::getBookedElementShareInvitations('protection',$bookingID);
		...
		{
			$link = JURI::root().'index.php?option=com_beseated&view=protectioninviteduserstatus&booking_id='.$bookingID.'&Itemid='.$Itemid;
