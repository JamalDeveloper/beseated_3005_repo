 ...
 * @since  0.0.1
 */
class BeseatedControllerYachtProfile extends JControllerAdmin
{
	/**
	 * Constructor.
	 *
	 ...
	 * @since   0.0.1
	 */
	public function getModel($name = 'YachtProfile', $prefix = 'BeseatedModel', $config = array('ignore_request' => true))
	{
	...
	public function change_currency()
	{
		$app           = JFactory::getApplication();
		$input         = $app->input;
		$yacht_id      = $input->get('yacht_id',0,'int');
		$currency_code = $input->get('currency_code','','string');

		if(!$yacht_id || empty($currency_code))
		{
			echo "400";
			exit;
		}
		...
		$model  = $this->getModel();
		$result = $model->saveYachtProfile($data,$yacht_id);
	...
	public function save()
	{
		...
		$yacht_id      = $input->get('yacht_id','','string');
		$yacht_name    = $input->get('yacht_name','','string');
		...
		$menuItem = BeseatedHelper::getBeseatedMenuItem('yacht-profile');
		$Itemid   = $menuItem->id;
	...
	public function uploadImage()
	{
		...
		$imagePath      = BeseatedHelper::uploadFile($file,'Yacht',$yacht->yacht_id);
		...
		if(!empty($imagePath) && in_array($orignalFile['extension'], $videoExtAllow))
		{
			...
			$storeImg =  'images/beseated/Yacht/'.$yacht->yacht_id.'/'.$orignalFile['filename'].'_png.png';
			...
			$storeFlv =  'images/beseated/Yacht/'.$yacht->yacht_id.'/'.$orignalFile['filename'].'_flv.flv';
			...
			$storeMp4 =  'images/beseated/Yacht/'.$yacht->yacht_id.'/'.$orignalFile['filename'].'_mp4.mp4';
			$command  = "/usr/bin/ffmpeg -i ".$destFlv." -ar 22050 ".$destMp4; //Wroking
			$output   = shell_exec($command);

			$destWebm  = $orignalFile['dirname'].'/'.$orignalFile['filename'].'_webm.webm';
			$storeWebm =  'images/beseated/Yacht/'.$yacht->yacht_id.'/'.$orignalFile['filename'].'_webm.webm';
			...
			$videoThumb     = JPATH_SITE.'/'.'images/beseated/Yacht/'.$yacht->yacht_id.'/';
			$videoimagePath = BeseatedHelper::createVideoThumb($videoPath, $videoThumb);
			$storeThumbPath = 'Yacht/'.$yacht->yacht_id.'/'.$videoimagePath;
		...
		{
			if(!JFolder::exists(JPATH_ROOT . "/images/beseated/Yacht/". $yacht->yacht_id . "/thumb"))
			{
				JFolder::create(JPATH_ROOT . "/images/beseated/Yacht/". $yacht->yacht_id . "/thumb");
			}
			$pathInfo       = pathinfo(JPATH_SITE.'/images/beseated/'.$imagePath);
			$thumbPath      = $pathInfo['dirname'].'/thumb/thumb_'.$pathInfo['basename'];
			$storeThumbPath = "Yacht/". $yacht->yacht_id . "/thumb/thumb_".$pathInfo['basename'];
		...
		if(!empty($imagePath))
		{
			...
			$tblImages->element_id   = $yacht->yacht_id;
			$tblImages->element_type = 'Yacht';
