 ...
 * @since  0.0.1
 */
class BeseatedControllerChauffeurProfile extends JControllerAdmin
{
	/**
	 * Constructor.
	 *
	 ...
	 * @since   0.0.1
	 */
	public function getModel($name = 'ChauffeurProfile', $prefix = 'BeseatedModel', $config = array('ignore_request' => true))
	{
	...
	public function change_currency()
	{
		$app           = JFactory::getApplication();
		$input         = $app->input;
		$chauffeur_id  = $input->get('chauffeur_id',0,'int');
		$currency_code = $input->get('currency_code','','string');

		if(!$chauffeur_id || empty($currency_code))
		{
			echo "400";
			exit;
		}
		...
		$model  = $this->getModel();
		$result = $model->saveChauffeurProfile($data,$chauffeur_id);
	...
	public function save()
	{
		...
		$chauffeur_id   = $input->get('chauffeur_id','','string');
		$chauffeur_name = $input->get('chauffeur_name','','string');
		...
		$data['chauffeur_name'] = $chauffeur_name;
		...
		$menuItem = BeseatedHelper::getBeseatedMenuItem('chauffeur-profile');
		$Itemid   = $menuItem->id;
	...
	public function uploadImage()
	{
		...
		$imagePath      = BeseatedHelper::uploadFile($file,'Chauffeur',$chauffeur->chauffeur_id);
		...
		if(!empty($imagePath) && in_array($orignalFile['extension'], $videoExtAllow))
		{
			...
			$storeImg =  'images/beseated/Chauffeur/'.$chauffeur->chauffeur_id.'/'.$orignalFile['filename'].'_png.png';
			...
			$storeFlv =  'images/beseated/Chauffeur/'.$chauffeur->chauffeur_id.'/'.$orignalFile['filename'].'_flv.flv';
			...
			$storeMp4 =  'images/beseated/Chauffeur/'.$chauffeur->chauffeur_id.'/'.$orignalFile['filename'].'_mp4.mp4';
			$command  = "/usr/bin/ffmpeg -i ".$destFlv." -ar 22050 ".$destMp4; //Wroking
			$output   = shell_exec($command);

			$destWebm  = $orignalFile['dirname'].'/'.$orignalFile['filename'].'_webm.webm';
			$storeWebm =  'images/beseated/Chauffeur/'.$chauffeur->chauffeur_id.'/'.$orignalFile['filename'].'_webm.webm';
			...
			$videoThumb = JPATH_SITE.'/'.'images/beseated/Chauffeur/'.$chauffeur->chauffeur_id.'/';

			$videoimagePath = BeseatedHelper::createVideoThumb($videoPath, $videoThumb);
			$storeThumbPath = 'Chauffeur/'.$chauffeur->chauffeur_id.'/'.$videoimagePath;
		...
		{
			if(!JFolder::exists(JPATH_ROOT . "/images/beseated/Chauffeur/". $chauffeur->chauffeur_id . "/thumb"))
			{
				JFolder::create(JPATH_ROOT . "/images/beseated/Chauffeur/". $chauffeur->chauffeur_id . "/thumb");
			}
			$pathInfo       = pathinfo(JPATH_SITE.'/images/beseated/'.$imagePath);
			$thumbPath      = $pathInfo['dirname'].'/thumb/thumb_'.$pathInfo['basename'];
			$storeThumbPath = "Chauffeur/". $chauffeur->chauffeur_id . "/thumb/thumb_".$pathInfo['basename'];
		...
		if(!empty($imagePath))
		{
			...
			$tblImages->element_id   = $chauffeur->chauffeur_id;
			$tblImages->element_type = 'Chauffeur';
